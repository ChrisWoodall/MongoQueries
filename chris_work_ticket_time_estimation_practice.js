db['oneWeek jobtickets'].aggregate(
	...
	[
		...
		// Stage 4
		{
			...
			jobcount: {"$sum": 1},
			workticket_startat: {"$min": "$jobstartat"},
			workticket_jobstarted: {"$min": "$jobstarted"},
			workticket_jobfinished: {"$max": "$jobfinished"},
		...
		// Stage 5
		{
			$project: {
			    ...
			    workticket_scheduled: "$workticket_startat",
			    workticket_started: "$workticket_jobstarted", 
			    workticket_jobfinished: "$workticket_jobfinished",
			    duration: 1,
			    waiting: 1,
			    total: 1,
			    jobs: 1
